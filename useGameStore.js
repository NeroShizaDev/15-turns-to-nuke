...
const createInitialGrid = () => {
  ...
  placeBuilding(grid, 20, 'barracks', 'p2');
...
export const useGameStore = create((set, get) => ({
  ...
  selectCell: (id, sourceMode = 'own') => {
    ...
    if (selected?.content?.owner === activePlayer) {
      ...
      if (areNeighbors(selectedCell, id)) {
        ...
        if (selected.content.type === 'infantry'
          && clicked.content && clicked.content.owner !== activePlayer && isTargetRevealed) {
          get().attackTarget(id);
          return;
        }
  ...
  attackTarget: (targetId) => {
    ...
    if (attackerCell.content.type !== 'infantry') return;
    ...
    if (targetCell.content.kind === 'unit') {
      newGrid[targetId] = clearContentAndViews(newGrid[targetId], turn);
      ...
      if (isCounterAttackPossible) {
        newGrid[selectedCell] = clearContentAndViews(newGrid[selectedCell], turn);
      ...
      newGrid[targetId] = !newGrid[targetId].content
